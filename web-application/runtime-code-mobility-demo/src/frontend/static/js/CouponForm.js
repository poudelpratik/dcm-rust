...
export default class CouponForm {
    ...
    #validateInput = async () => {
        try {
            const returnValue0 = await validate_coupon_input(
                this.input.value
            );
            if (returnValue0 === false) {
                this.input.focus();
                return [returnValue0, "Input must be 11 characters long."];
            }
            ...
            return [
                false,
                'An error has occurred. Vouchers cannot currently be used.',
            ];
        }
    };
    ...
    #validateCoupon = async () => {
        try {
            const valid = await validate_coupon(
                this.input.value
            );

            if (!valid) {
                return [valid, 'Invalid coupon.'];
            }
