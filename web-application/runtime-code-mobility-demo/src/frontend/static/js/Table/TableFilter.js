import {sort_floats, sort_strings} from '../CodeDistributor/exports.js';
import {addEventListenerMulti, selectElement, selectElements,} from '../utils/helpers.js';
...
export default class Filter {
    ...
    wasmSort = async (products, criteria, sortMode) => {
        ...
        if (typeof filtered[0] === 'string') {
            filtered = await sort_strings(
                sortMode,
                filtered
            );
        } else {
            filtered = await sort_floats(
                sortMode,
                filtered
            );
        }
